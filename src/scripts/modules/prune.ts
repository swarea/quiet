...
function humaniseTodayStamps(): void {
  document.querySelectorAll<HTMLElement>(".quiet-post-row time, .quiet-side-list time").forEach((el) => {
...
export function initPrune(): void {
  ...
  document.querySelectorAll<HTMLElement>(".quiet-rail nav").forEach((nav) => {
  ...
  document.querySelectorAll<HTMLElement>(".quiet-recent").forEach((list) => {
    if (list.querySelector(".quiet-post-row")) return;
    const box = document.createElement("div");
    box.className = "quiet-empty";
    box.lang = "en";
    box.innerHTML =
